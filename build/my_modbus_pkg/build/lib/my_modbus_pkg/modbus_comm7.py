...
class ModbusNode(Node):
    ...
    def rotate_to_position(self, motor_address, target_position):
        ...
        while current_position != target_position:
            current_position = self.get_encoder_position(motor_address)
            if current_position is None:
                self.get_logger().error(f"Failed to read encoder position during rotation for motor {motor_address}")
    ...
    def calculate_velocity_from_position_difference(self, position_difference):
        ...
        # For simplicity, we're using a proportional control (P-control)
...
def main(args=None):
    ...
    while rclpy.ok():
        motor_input = input("Enter motor number (1, 2, or both) to control: ").strip().lower()
        ...
        if command == 'enable':
            ...
            print("Unknown command. Please enter 'enable', 'disable', 'startjog', 'stopjog', 'setjogvelocity', 'rotate', or 'exit'.")
